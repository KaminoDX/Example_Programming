	...
	<body>
		...
		if(count($_POST)>0) {
			...
			if(strlen($_POST["NN"])>0) {
				$arr[] = "tbl_kunden.Nachname='" . $_POST["NN"] . "'";
			}
			if(strlen($_POST["ReNo"])>0) {
				$arr[] = "tbl_rechnungen.ReNo='" . $_POST["ReNo"] . "'";
			}
		...
		$sql = "
			...
			INNER JOIN tbl_zahlungsarten ON tbl_zahlungsarten.IDZahlungsart=tbl_rechnungen.FIDZahlungsart
